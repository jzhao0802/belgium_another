...
var_list <- setdiff(colnames(raw_data), c(persis_vars))
...
further_to_deleted<- ordinal_list[pos_num<100] #88  # that is for logistic regression
#drop Yan's covariates
Yan_var <- grep('^pre_', var_list, value=T)

#get the clean data to move to the modeling part
raw_data_clean<- raw_data[,setdiff(var_list,c(cons_var, further_to_deleted, Yan_var))]
dim(raw_data_clean)#[1] 65087   282
names(raw_data_clean)

#output the covariates list into model
covar_forModel <- setdiff(names(raw_data_clean), 'response')
covar_froModel_df <- variable_screen_list[match(covar_forModel, variable_screen_list[, 1]), ]
write.xlsx(covar_froModel_df, 'Covar_list_forModel_delYan_100_forLogistic.xlsx', row.names=F)
#check the correlation between covariates
get_corr <- function(thresh){
    corr_matrix <- cor(raw_data_clean)
    write.xlsx(corr_matrix, 'Correlation Result.xlsx', sheetName='matrix', row.names=T, append=T, showNA=T)
    pdf('Correlation_matrix.pdf', height=6, width=8, pointsize=12)
    heatmap(corr_matrix)
    dev.off()
    
    add_r <- numeric()
    high_corr <- numeric()
    for(i in 1:(ncol(raw_data_clean)-1)){
        for(j in (i+1):ncol(raw_data_clean)){
            if(abs(corr_matrix[i, j]) > thresh){
                add_r <- c(add_r, add=T)
                high_corr <- rbind(high_corr, c(Var1=names(raw_data_clean)[i], Var2=names(raw_data_clean)[j], Corr=corr_matrix[i, j]))
            }
        }
    }
    high_corr <- as.data.frame(high_corr)
    write.xlsx(high_corr, 'Correlation Result.xlsx', sheetName='high_corr', row.names=T, append=T, showNA=T)
    return(high_corr)
}
high_corr_result <- get_corr(0.5)
