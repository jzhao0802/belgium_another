...
Run_BE_RF_Model<- function(all_samples_forsim, par_all_scenarios, simulations,out_path, cutoff_score, Trial, logF){
    ...
    for(i in 1:simulations){
        ...
        temp_out<-foreach(scen = 1:nrow(par_all_scenarios), .packages=c('randomForest','glmnet'), .combine='c') %dopar% {
            ...
            vl_auc<-auc(as.numeric(as.vector(validation_data$response)),vl_pred[,2])
            return(vl_auc)
        ...
        full_tr_auc<- auc(as.numeric(as.vector(full_training_data$response)), full_tr_pred[,2])
        
        test_pred <- predict(full_RF_fit, test_data,  type='prob')
        test_auc <- auc(as.numeric(as.vector(test_data$response)), test_pred[,2])
